...
export default async function PaymentsList() {
  ...
  const { data } = await supabase
    .from("payments")
    .select("id, amount, paid_at, method, note, visit_id, clients(name), visits(invoice_no)")
  ...
  const standalone = rows.filter((r) => !r.visit_id);
  const linked     = rows.filter((r) =>  r.visit_id);

  return (
    <div className="pb-4">
      <div className="px-4 py-3 flex items-center justify-between">
        <h2 className="font-cairo font-bold text-ink text-base">سندات القبض ({rows.length})</h2>
        <Link href="/payments/new" className="flex items-center gap-1 text-xs font-cairo font-semibold text-white bg-primary px-3 py-2 rounded-full">
          <Plus size={14} /> سند قبض جديد
        </Link>
      </div>

      {rows.length === 0 ? (
        <EmptyState icon={Receipt} title="لا توجد سندات قبض" subtitle="سجّل أوّل تحصيل من زبون" ctaHref="/payments/new" ctaLabel="سند قبض جديد" />
      ) : (
        <>
          {standalone.length > 0 && (
            <div className="px-3 mb-4">
              <h3 className="text-[11px] font-cairo font-bold text-muted uppercase mb-2 px-1">تحصيلات مستقلة (تسديد ذمم) ({standalone.length})</h3>
